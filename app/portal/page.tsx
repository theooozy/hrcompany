...
{ name: '찐예쁨', platform: 'YouTube · IG · TikTok', subscribers: '1.2만 / 900 / 1,562', makePrice: 70, postPrice: 30, link: 'https://www.youtube.com/@%EC%98%88%EB%BB%90%EC%A7%80%EA%B3%A0%EC%8B%B6%EB%8B%88' },
{ name: '미모지상주의', platform: 'YouTube · IG · TikTok', subscribers: '6.21만 / 2 / 2,683', makePrice: 70, postPrice: 30, link: 'https://www.youtube.com/@%EC%98%88%EB%BB%90%EC%A7%80%EA%B3%A0%EC%8B%B6%EC%9C%BC%EB%A9%B4%EA%B5%AC%EB%8F%85' },
...
{ name: '신기+템', platform: 'YouTube · IG · TikTok', subscribers: '14.4만 / 1,759 / 2,343', makePrice: 65, postPrice: 30, link: 'https://www.youtube.com/@%EC%9D%B4%EB%BB%90%EC%A7%80%EA%B3%A0%EC%8B%B6%EC%9C%BC%EB%A9%B4%EA%B5%AC%EB%8F%85' },
...
const displayName = ch.isJapan && JP_CHANNEL_KR[ch.name]
? ch.name + '(' + JP_CHANNEL_KR[ch.name] + ')'
: ch.name;
return (
...
{ch.link && (
<a href={ch.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 mb-3 text-xs text-red-600 hover:text-red-700 font-medium bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-all w-fit">
{'▶'} 유튜브 채널 바로가기
</a>
)}
...
<h2 className="text-xl md:text-3xl font-extrabold text-white mb-3">진행을 원하시나요?</h2>
<p className="text-blue-100 mb-8 text-base">세부 정보를 입력해주시면 담당자가 곧 연락드립니다.</p>
<a href="/portal/details" className="block w-full sm:w-auto sm:inline-block px-6 py-3 bg-white text-blue-700 rounded-2xl font-bold text-base shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 text-center">
세부 정보 입력하기 →
</a>
</div>
...
<a href="/portal/details" className="inline-block px-8 py-3 bg-white text-blue-700 rounded-2xl font-bold shadow-md hover:shadow-lg hover:scale-105 transition-all duration-200">
세부 정보 입력하기 →
</a>
</div>
