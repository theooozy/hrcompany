...
const [addPanelChannels, setAddPanelChannels] = useState<string[]>([]);
  const [addPanelForm, setAddPanelForm] = useState({
    ...
    material: '', secondary_use: '', work_type: '콘티', work_status: '', memo: ''
  });
...
<div className="w-full" ref={channelRef}>
<div className="flex flex-wrap gap-1 mb-1">
{editChannels ? editChannels.split(',').map(c => c.trim()).filter(Boolean).map(c => (
<span key={c} className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">
{getChLabel(c)}
<button type="button" onClick={(e) => { e.stopPropagation(); const newChs = editChannels.split(',').map(x=>x.trim()).filter(x=>x&&x!==c).join(','); setEditChannels(newChs); autoSave('channels', newChs||null); }} className="text-blue-400 hover:text-red-500 font-bold ml-0.5">×</button>
</span>
)) : <span className="text-slate-400 text-sm">없음</span>}
</div>
<div className="flex items-center gap-1">
<select onChange={(e) => { const val = e.target.value; if (!val) return; e.target.value=''; const existing = editChannels ? editChannels.split(',').map(x=>x.trim()).filter(Boolean) : []; if (!existing.includes(val)) { const newChs = [...existing, val].join(','); setEditChannels(newChs); autoSave('channels', newChs); } }} defaultValue="" className="flex-1 text-sm text-slate-800 bg-white border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400">
<option value="">추가...</option>
{channelOptions.map(c => <option key={c} value={c}>{getChLabel(c)}</option>)}
</select>
</div>
</div>
} />
...
/>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex" onClick={() => { setStatusDropdown(null); }}>
                  ...
                  <div className="border-t border-slate-100 p-5 space-y-4">
                    ...
                    <div className="flex items-center gap-3 px-1 pt-2">
                      ...
                      {(!inq.ad_review_status || inq.ad_review_status === 'pending') && (
                        <>
                          <input type="datetime-local" value={selectedDates[inq.id] || new Date().toISOString().substring(0,16)} onChange={(e) => setSelectedDates({...selectedDates, [inq.id]: e.target.value})} className="ml-auto px-2 py-1 border border-slate-300 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-green-400" />
                          <button onClick={() => { const dt = selectedDates[inq.id] || new Date().toISOString().substring(0,16); handleAdReview(inq.id, 'approved', dt); }} className="px-4 py-1.5 bg-green-500 hover:bg-green-600 text-white rounded-lg text-xs font-semibold">승인</button>
      ...
      {showAddPanel && (
        <>
          <div className="fixed inset-0 bg-black/30 z-40" onClick={() => { setShowAddPanel(false); setAddPanelError(''); setAddPanelChannels([]); setAddPanelForm({ product_name: '', brand_name: '', channel: '', manager_name: '', deadline: '', status: '진행중', youtube_url: '', email: '', phone: '', business_number: '', product_link: '', material: '', secondary_use: '', work_type: '콘티', work_status: '', memo: '' }); }} />
          <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-lg font-bold text-slate-800">새 일정 추가</h2>
              <button onClick={() => { setShowAddPanel(false); setAddPanelError(''); setAddPanelChannels([]); setAddPanelForm({ product_name: '', brand_name: '', channel: '', manager_name: '', deadline: '', status: '진행중', youtube_url: '', email: '', phone: '', business_number: '', product_link: '', material: '', secondary_use: '', work_type: '콘티', work_status: '', memo: '' }); }} className="text-slate-400 hover:text-slate-700 text-2xl leading-none">×</button>
            </div>
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">채널 <span className="text-xs font-normal text-slate-400">(복수 선택 가능)</span></label>
                <div className="flex flex-wrap gap-1 mb-2 min-h-[28px] p-2 border border-slate-300 rounded-lg bg-slate-50">
                  {addPanelChannels.length === 0 && <span className="text-xs text-slate-400">없음</span>}
                  {addPanelChannels.map(c => (
                  <span key={c} className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">
                  {getChLabel(c)}
                  <button type="button" onClick={() => setAddPanelChannels(prev => prev.filter(x => x !== c))} className="text-blue-400 hover:text-red-500 font-bold ml-0.5">×</button>
                  </span>
                  ))}
                </div>
                <select onChange={(e) => { const val = e.target.value; if (!val || addPanelChannels.includes(val)) return; e.target.value=''; setAddPanelChannels(prev => [...prev, val]); }} defaultValue="" className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                  <option value="">추가...</option>
              ...
              <div>
                ...
                <textarea value={addPanelForm.memo} onChange={e => setAddPanelForm(f => ({ ...f, memo: e.target.value }))} placeholder="메모를 입력하세요..." rows={3} className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
              </div>
